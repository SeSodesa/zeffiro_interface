...
persistent lastJobIDQueried
persistent timeLastQueried
...
% Adding in a buffer to stop us querying every 1 second due to job.wait().
% If we recently queried this job then we return the old state. If we have
% waited more than the timer then we check to see if the state has updated.
updateLag = validatedPropValue(cluster, 'UpdateLag', 'double');
if ~isempty(lastJobIDQueried) && lastJobIDQueried==job.ID
if seconds(timeLastQueried-datetime('now')) <= updateLag
% Too soon the check again
return
end
end
remoteConnection = getRemoteConnection(cluster);
[schedulerIDs, numSubmittedTasks] = getSimplifiedSchedulerIDsForJob(job, data);
...
lastJobIDQueried = job.ID;
timeLastQueried = datetime('now');
...
numFailed   = numel(regexp(sacctOut, 'CANCELLED|FAILED|TIMEOUT|PREEMPTED|NODE_FAIL|OUT_OF'));
