...
GMM_number=3+1; %number of GMModels (plus 1 for the maximum)
...
gLab={'max', 'gmm\_25','gmm\_50', 'gmm\_75'};
...
myXLabel{(m-1)*4 +1}=gLab{1};
myXLabel{(m-1)*4 +2}=strcat(gLab{2}, '\newline', mLab{m});
myXLabel{(m-1)*4 +3}= gLab{3};
...
%determine colors
c1=nan(length(plottingStuff.eeg.matSuperBig), 3);
...
figure('Position',[120,20, 1200, 800]);
hold on;
title(strcat(nom, '-', 'Distance to resection'));
bar(plottingStuff.(nom).matSuperBig);
