...
function [shape_param, scale_param, snr_vec] = find_g_hyperprior( ...
    L, ...
    snr_val, ...
    tail_length_db, ...
    source_space_size, ...
    ...
    % find_g_hyperprior
    %
    % Forms the gamma (γ) hyper-prior information needed by inverse methods
    % such as RAMUS.
    %
    ...
    for j = 1 : 10
        ...
        p_val_vec = goodness_of_inversion.gamma_gpu(relative_noise_std.^2.*tail_length.^2,shape_param_vec,relative_noise_std(:, ones(size(shape_param_vec,2),1)).^2 ./ (shape_param_vec));
